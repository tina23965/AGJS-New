...
function checkPwStrong(pwd){
    var strong = '';
    //大寫英文
    var lenghtS = getPwValue(pwd, 's');
    //小寫英文
    var lenghtX = getPwValue(pwd, 'x');
    //數字
    var lenghtI = getPwValue(pwd, 'i');
    

    if (pwd.length >= 4 && pwd.length <= 9){
            strong = 1;
            if(lenghtS >= 1 && lenghtX >= 2 && lenghtI >= 1){
                strong = 2;
            }
    }else if (pwd.length >= 10 && pwd.length <= 11){
            strong = 2;
            if(lenghtS >= 2 && lenghtX >= 4 && lenghtI >= 4){
                strong = 3;
            }
    }else if (pwd.length >= 12 && lenghtS >= 2 && lenghtX >= 4 && lenghtI >= 4){
            strong = 3;
    }
...
//檢查密碼所屬type的長度 
function getPwValue(pw, type) {
    //type=s檢查大寫英文 i檢查數字 x檢查小寫英文
    var score_i = 0, score_s = 0, score_x = 0;
